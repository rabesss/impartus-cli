...
func TestWatcherRedactsLectureFailureDetails(t *testing.T) {
	...
	producer := &fakeProducer{
		...
		fetchErrors: map[int]error{lecture.TTID: errors.New("Authorization: Token abc token=secret-value")},
	}
	var output bytes.Buffer
	...
	if strings.Contains(err.Error(), "Token abc") || strings.Contains(err.Error(), "secret-value") {
	...
	for _, event := range decodeEvents(t, output.String()) {
		...
		if strings.Contains(encoded, "Bearer abc") || strings.Contains(encoded, "secret-value") {
