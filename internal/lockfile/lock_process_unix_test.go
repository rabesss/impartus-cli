...
func TestLockIsReleasedAfterSIGKILL(t *testing.T) {
	path := filepath.Join(t.TempDir(), "artifact.lock")
	command := exec.CommandContext(context.Background(), os.Args[0], "-test.run=TestLockHelperProcess") // #nosec G204 -- test binary and fixed argument
	command.Env = append(os.Environ(), "IMPARTUS_TEST_LOCKFILE_HELPER=1", "IMPARTUS_TEST_LOCKFILE_PATH="+path)
	...
	if err != nil {
		t.Fatalf("TryAcquire(after SIGKILL) error = %v", err)
	}
...
func TestLockHelperProcess(t *testing.T) {
	if os.Getenv("IMPARTUS_TEST_LOCKFILE_HELPER") != "1" {
		return
	}
	lock, err := TryAcquire(os.Getenv("IMPARTUS_TEST_LOCKFILE_PATH"))
	if err != nil {
		os.Exit(2)
	}
	defer func() {
