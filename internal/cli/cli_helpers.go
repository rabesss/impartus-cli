...
func showHelp(version, date string) {
	...
	fmt.Println("  watch [flags]                        Poll for new lectures and upload audio to NotebookLM")
	...
	fmt.Println("  --subject,-s        Subject ID")
	fmt.Println("  --session,-S        Session ID")
	fmt.Println("  --interval          Poll interval (default 5m)")
	fmt.Println("  --state             State file path (default ./.watch-state.json)")
	fmt.Println("  --notebook          NotebookLM notebook ID")
	...
	fmt.Println("  --check             Validate ffmpeg/auth/config then exit")
