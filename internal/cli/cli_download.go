...
func executeDownloadWithDependencies(args []string, presentation downloadPresentationOptions, deps downloadExecutionDependencies) (downloadResult, error) {
	...
	if scopeErr := resolveDownloadLectureScope(ctx, cfg, apiClient, selected, f.subject, f.session); scopeErr != nil {
...
func resolveDownloadLectureScope(ctx context.Context, cfg *config.Config, catalog courseCatalog, lectures client.Lectures, subjectID, sessionID int) error {
...
func applySelectedCourseScope(lectures client.Lectures, subjectID, sessionID int) (map[int]struct{}, bool) {
...
func resolveInstituteFromCatalog(ctx context.Context, cfg *config.Config, catalog courseCatalog, subjectID, sessionID int) (int, error) {
	if catalog == nil {
