...
func TestWaitForEndCancellationClosesIPCAndDrainsInFlightFailure(t *testing.T) {
	clientConnection, peerConnection := net.Pipe()
	t.Cleanup(func() {
		_ = peerConnection.Close() //nolint:errcheck
	})
	...
	select {
	case err := <-waitDone:
		t.Fatalf("WaitForEnd returned %v before cancellation drained the in-flight failure", err)
	case <-time.After(50 * time.Millisecond):
	}
	close(releaseHandler)
	select {
