...
- golangci-lint is installed at `$(go env GOPATH)/bin/golangci-lint` (v1.64.8, built with Go 1.24). The project uses Go 1.24.0, which may cause `typecheck` linter false positives from standard library version mismatch. Use `$(go env GOPATH)/bin/golangci-lint` rather than expecting it on PATH. The `make lint` target handles the fallback automatically.
