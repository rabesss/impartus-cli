...
* [Impartus CLI](#impartus-cli)
  ...
  * [CLI Usage](#cli-usage)
    ...
    * [Durable Watch](#durable-watch)
    * [Local Lecture Library](#local-lecture-library)
...
| `--reuse-verified` | | Reuse a verified local artifact after digest and container checks. Requires `--ttid`, or both `--start` and `--end` spanning at most two lectures. Missing or corrupt files, or a busy per-artifact lock after 60s, fall back to a normal download. `--output` applies only to lectures that still download. Reused lectures keep their stored paths. | Download Only |
